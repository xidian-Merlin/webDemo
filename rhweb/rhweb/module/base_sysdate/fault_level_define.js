...
require(['/js/plugin/inputdrop/inputdrop.js'] ,function (inputdrop){
	...
	// 故障等级定义列表URL
	var list_url = "FaultLevel/queryFaultLevel";
	var list_col = [
						{text:'事件名称',name:"name"},
	...
	var list_oper = [{icon:"rh-icon rh-edit",text:"修改",aclick:fault_edit}];
	...
	fault_level_list({paramObj:null,isLoad:true,maskObj:"body"});
	...
	function fault_level_list(option)
	{
	...
	function fault_edit(rowData)
	{
		$.ajax({
			...
			success :function(data)
			{
				g_dialog.dialog($(data).find("[id=fault_level_edit_template]"),{
					width:"550px",
					init:init,
					title:"故障事件等级定义修改",
					saveclick:save_click
