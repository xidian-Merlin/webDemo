...
var index_list_col_header = [
							  ...
							  {text:'代理服务器',name:"enftpStatus",render:function(text){
									if (text ==1) {
										return '启用';
									}
									else{
							  ...
							  {text:'策略状态',name:"eventType",render:function(text){
									if (text ==1) {
										return '安全事件';
									}
									else if(text == 2){
							  ...
							  {text:'修改时间',name:"enterDate",searchRender:function (el){
								index_render_div(el ,{type:"date", startKey:"enterStartDate" ,endKey:"enterEndDate"});
							  }},
							  {text:'备注',name:"createUser",hideSearch:true}
							];
...
function edit_template_init(rowData)
{
	...
	function init(el)
	{
		...
		el.find("[id=chevron-right1]").click(function (){
			...
			var buffer = [];
			var obj = new Object();
			obj.ipTypeN = el.find("[data-id=emailTitle]").val();
			buffer.push(obj);
		...
		el.find("[id=chevron-left1]").click(function (){
			...
			g_grid.addData(el.find("[data-id=emailTitle]") ,data);
		});
		...
		// 第三个list
		g_formel.code_list_render({key:"faultclass",faultEventTypeEl:el.find("[data-id=eventTypeComboBox3]"),screen:screen});
		el.find("[data-id=eventType]").change(function(){
			el.find("[data-id=eventTypeComboBox3]").find("option").remove();
			if($(this).attr("data-flag") == "false")
			{
				g_grid.removeData(el_table_right2 ,{});
			}
			...
			{
				g_formel.code_list_render({key:"perfclass",performEventTypeEl:el.find("[data-id=eventTypeComboBox3]"),screen:screen});
			}
			sample_list_get("-1");
			$(this).attr("data-flag","false");
		});
		el.find("[data-id=eventTypeComboBox3]").change(function(){
			var eventType = el.find("[data-id=eventTypeComboBox3]").val();
			sample_list_get(eventType);
		});
		
		g_grid.render(el_table_right3,{
			 header:[{text:"样本名称",name:"eventName"}],
			 data:[],
			 hideSearch:true,
			 paginator:false
		});
	...
	function initAfter(el)
	{
		...
		var ipList_table1 = el.find("[id=ipList_table1]");
		var ipList_table = el.find("[id=ipList_table4]");
	...
	function sample_list_get(eventType)
	{
		var el = $("#edit_template");
		var typeValue = el.find("[data-id=eventType]").val();
		var eventType = eventType;
		var eventStore = g_grid.getData(el.find("[id=eventList_right_table3]"));
...
function edit_template_render(el ,data ,rowData)
{
	...
	if(rowData.eventType ==1){
		...
		// 回显IPLIST
		g_grid.addData(el.find("[id=ipList_table4]") ,data.ipLists);
