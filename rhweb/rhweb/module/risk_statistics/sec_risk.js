...
function event_init()
{
	...
	$("#export_btn").click(function (){
		window.location.href = index_web_app + "domainRisk/exportRiskInfoSD";
	});
}
	...
	$("#accordion_icon").find("div").click(function (){
		...
		um_ajax_post({
			...
			successCallBack : function (data){
				data.securityDomainTreeStore.splice(0,0,{id:"-1" ,parentID:"" ,label:"全部"});
				tree.render($("#accordion"),{
					zNodes : data.securityDomainTreeStore,
...
function list_list(paramObj ,opt)
{
	g_grid.render($("#table_div"),{
		"header":[
				  {text:'安全域全名',name:"domaName" ,width:62.5},
		 ...
		 paramObj:paramObj,
		 url:"domainRisk/queryDomainRiskSD",
		 isLoad:false,
		 allowCheckBox:false,
...
function line_chart_render(rowData)
{
	...
	$("#sec_area").oneTime(600 ,function (){
		um_ajax_post({
			...
			paramObj : {type:$("#time_sel").val() ,domaId :rowData.domaId ,objType:1},
			successCallBack : function (data){
				var	categoryArray = [];
				var	seriesArray = [];
...
function daomin_alerm_get(rowData)
{
	um_ajax_get({
				...
				paramObj : {queryTime:$("#time_sel").val() ,domaId :rowData.domaId ,objType:1 ,firstNum:0 ,timeLable:rowData.timeLable},
				successCallBack : function (data){
