...
+(function($){
	...
	Calendar.prototype.init = function(element, options){
		this.date = this.getDate(options);
		...
		if( options.select ){
			$(element).on('click', 'table>tbody>tr>td', options, function(){
				...
				if( options.onselect ){
					options.onselect.call(options.target, $(this).data('time'), this);//添加被点击空间对象
				}
			})
		}
	...
	Calendar.prototype.bulidWeekly = function(options, year ,month){
		...
		for(var j = 0; j < 6; j++ ){
			if( j != 0 && parseInt(startDay) >= parseInt(endDay) ){//结束天小于开始天，并且不为第一周
				if( selectDate ){
					crt = 'week' + ( j + 1 );
				...
				if( selectDate ){
					crt = 'week' + ( j + 1 );
					if( ($.isArray(selectDate[2]) && ((year == selectDate[2][0][0] && month == selectDate[2][0][1] && selectDate[2][0][2] == crt) || ( false&&selectDate[2][1] && year == selectDate[2][1][0] && month == selectDate[2][1][1] && selectDate[2][1][2] == crt))) ||  ( year == selectDate[0] && month == selectDate[1] && selectDate[2] == crt )  ){
						calendar += '<tr><td class="current-week current-day selected tweek week'+(j + 1)+'" data-time="'+startYear+'-'+this.dispose(startMonth)+'-'+this.dispose(startDay)+','+endYear+'-'+this.dispose(month)+'-'+this.dispose(endDay)+'" colspan="7"><div><span class="week">第'+ numbers[j] +'周</span><span class="process">'+this.dispose(startMonth)+'/'+this.dispose(startDay)+'~'+this.dispose(month)+'/'+this.dispose(endDay)+'</span></div></td></tr>';						
					}else{
	...
	Calendar.prototype.getDate = function(options){
		var now = new Date();
		if(options.selectDate){
			var dateStr = options.selectDate + " 00:00:00";
