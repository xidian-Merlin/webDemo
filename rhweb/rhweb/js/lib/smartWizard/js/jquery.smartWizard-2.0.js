    ...
    $.fn.smartWizard = function(action) {
        ...
        return this.each(function(){
                ...
                function init(){
                  ...
                  btnMask = $('<div class="mask" style="display:none;opacity:0"></div>');
                ...
                function prepareSteps(){
                  ...
                  if (options.isMonitorDetail)
                  {
                    $(steps, obj).eq(0).removeClass("selected").removeClass("disabled").addClass("done"); 
                    $(steps, obj).eq(0).attr("isDone",1);
                    $(steps, obj).eq(1).removeClass("selected").removeClass("disabled").addClass("done"); 
                    $(steps, obj).eq(1).attr("isDone",1);
                    $(steps, obj).eq(2).removeClass("selected").removeClass("disabled").addClass("done"); 
                    $(steps, obj).eq(2).attr("isDone",1);
                    $(steps, obj).eq(3).addClass("selected").removeClass("disabled").removeClass("done"); 
                    $(steps, obj).eq(3).attr("isDone",0);
                  }
                ...
                function showStep(stepIdx){
                    ...
                    var umDialog = selStep.closest("[data-id*=umDialog]");
   
                    var umDialogHeaderHeight = umDialog.find("[class*=modal-header]").outerHeight();

                    var elmStepContainerHeight = umDialog.outerHeight()
                                                  - umDialogHeaderHeight - 150;
                    // elmStepContainerHeight = (elmStepContainerHeight > 400 ? 400 : elmStepContainerHeight);

                    elmStepContainer.height(elmStepContainerHeight);               
