...
var g_grid = {
		render : function (el ,opt){
			...
			$.ajax({
				type: "GET",
				url: "/tdyth/resources/plugins/grid/grid.html",
		...
		renderData : function (el ,opt){
			...
			if (opt.paginator)
			{
				opt.paramObj.currentPage = opt.currentPage;
			
				opt.paramObj.numPerPage = opt.pageSize;
			...
			else
			{
				/*var type = "";
				var id = 0;
				if(opt.nodeId == "root") {type = "root"; id = 0;}
				else if(opt.nodeId == -1) {type = "class"; id = 1;}
				else if(opt.nodeId > 0) {type = "type"; id = opt.nodeId}*/
				
				$.ajax({
					type: "POST",
					url: opt.url,
					dataType: "json",
					data:opt.paramObj, /*{type:type,id: id,currentPage:opt.currentPage,numPerPage:opt.pageSize}*/
					success :function(data){
			/*	um_ajax_get({
					url : opt.url,
					isLoad : opt.isLoad,
					...
					successCallBack : function (data)*/
						...
						var dataList;
						if(data.assetDetailsList) dataList = data.assetDetailsList;
						if(data.infoList) dataList = data.infoList;
						if(data.resultList) dataList = data.resultList;
						inFunc_render(dataList);
		...
		renderTr : function (el ,opt ,data ,emptyFlag){
			...
			for (var i=0;i<data.length;i++)
			{
				...
				for (var j=0;j<header.length;j++)
				{
					...
					if(header[j].name == 'description') {
						buffer.push('<td><div style="word-wrap: break-word;text-align:left;font-weight:'+header[j].bold+'" '+titleText+'>'+tdText+'</div></td>');}
					else buffer.push('<td><div style="word-wrap: break-word;text-align:'+header[j].align+';font-weight:'+header[j].bold+'" '+titleText+'>'+tdText+'</div></td>');
				}
				// 渲染操作栏
				...
				//el.find("[data-id=um-grid-table-div]").find("table").append("dfasddfsf");
		...
		initPager : function (el ,opt){
			...
			el.find("[data-id=total_record_count]").text(totalRows);
