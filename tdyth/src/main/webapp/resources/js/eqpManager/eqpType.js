...
var index_list_col_header = [
					  ...
					  {text : "图片",name : "pic", width:10, render:function (txt ,rowData){
						  return '<div class="prel" style="height:22px">'
							+'</span><img src="/tdyth/resources/images/topo/'
							+rowData.pic+'.png'+'" title="'+rowData.pic+'" style="width:20px; height:20px;vertical-align:middle"></img></div>';
			}}
					];
...
{
	var target_title = rowData ? "设备类型修改" : "设备类型添加";
	var url = rowData? "/tdyth/resources/js/eqpManager/eqpType_tp1.html" : "/tdyth/resources/js/eqpManager/eqpType_tp1_1.html"
	...
	function init(el)
	{
		...
		if(rowData){
			el.find("[data-id=deviceTypeName]").val(rowData.typeName);
		}
	}
...
function detail_template_init(rowData)
{
	...
	/*$.ajax({
		type: "GET",
		url: "/tdyth/resources/js/Manager/eqpType_tp1.html",
		success :function(data)
		{
